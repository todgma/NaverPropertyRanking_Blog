...
/// <summary>
/// SEED 블록암호(KISA 표준, RFC 4269)의 CBC 암호화.
/// 이실장 로그인이 쓰는 ISSAC WebCrypto가 대칭키 알고리즘으로 SEED를 요구해서 넣었다.
/// 복호화는 쓸 일이 없어 암호화만 구현한다.
/// </summary>
public static class SeedCipher
{
    ...
    /// <summary>키 스케줄 상수(KC). 표준이 정한 고정값이다.</summary>
    ...
    /// <summary>G 함수용 치환표. 표준이 정한 고정값이다.</summary>
    ...
    /// <summary>
    /// SEED-CBC로 암호화한다. 블록이 딱 맞아도 한 블록을 더 붙이는 PKCS#7 패딩을 쓴다.
    /// </summary>
    public static byte[] EncryptCbc(byte[] plain, byte[] key, byte[] iv)
    {
        ...
        if (key.Length != KeySize) throw new ArgumentException($"키는 {KeySize}바이트여야 합니다.", nameof(key));
        if (iv.Length != BlockSize) throw new ArgumentException($"IV는 {BlockSize}바이트여야 합니다.", nameof(iv));

        var roundKeys = ExpandKey(key);
        var padded = AddPadding(plain);
        var result = new byte[padded.Length];
        var chain = (byte[])iv.Clone();

        for (var offset = 0; offset < padded.Length; offset += BlockSize)
        {
            var block = new byte[BlockSize];
            for (var i = 0; i < BlockSize; i++) block[i] = (byte)(padded[offset + i] ^ chain[i]);
            var encrypted = EncryptBlock(block, roundKeys);
            Array.Copy(encrypted, 0, result, offset, BlockSize);
            chain = encrypted;
        }
    ...
    /// <summary>PKCS#7 패딩을 붙인다.</summary>
    private static byte[] AddPadding(byte[] plain)
    {
        var padding = BlockSize - plain.Length % BlockSize;
        var padded = new byte[plain.Length + padding];
        Array.Copy(plain, padded, plain.Length);
        for (var i = plain.Length; i < padded.Length; i++) padded[i] = (byte)padding;
        return padded;
    }

    /// <summary>G 함수. 바이트 단위로 쪼개 치환표 네 개를 XOR한다.</summary>
    private static uint G(uint value) =>
        SS3[(value >> 24) & 0xFF] ^ SS2[(value >> 16) & 0xFF] ^
        SS1[(value >> 8) & 0xFF] ^ SS0[value & 0xFF];

    /// <summary>키 16바이트에서 라운드키 16쌍을 만든다.</summary>
    private static uint[,] ExpandKey(byte[] key)
    {
        var a = ReadUInt32(key, 0);
        var b = ReadUInt32(key, 4);
        var c = ReadUInt32(key, 8);
        var d = ReadUInt32(key, 12);

        var roundKeys = new uint[16, 2];
        for (var round = 0; round < 16; round++)
        {
            roundKeys[round, 0] = G(unchecked(a + c - Kc[round]));
            roundKeys[round, 1] = G(unchecked(b - d + Kc[round]));

            // 홀수 라운드는 A·B를, 짝수 라운드는 C·D를 8비트씩 굴린다.
            if (round % 2 == 0)
            {
                var previous = a;
                a = (a >> 8) | (b << 24);
    ...
    /// <summary>16라운드 파이스텔 구조로 한 블록을 암호화한다.</summary>
    private static byte[] EncryptBlock(byte[] block, uint[,] roundKeys)
    {
        var left0 = ReadUInt32(block, 0);
        var left1 = ReadUInt32(block, 4);
        var right0 = ReadUInt32(block, 8);
        var right1 = ReadUInt32(block, 12);

        for (var round = 0; round < 16; round++)
        {
            var t0 = right0 ^ roundKeys[round, 0];
            var t1 = right1 ^ roundKeys[round, 1];
            t1 ^= t0;
            t1 = G(t1);
            t0 = unchecked(t0 + t1);
            t0 = G(t0);
            ...
            var nextRight0 = left0 ^ t0;
            var nextRight1 = left1 ^ t1;
            left0 = right0;
            left1 = right1;
            right0 = nextRight0;
            right1 = nextRight1;
        ...
        // 마지막 라운드의 교환을 되돌려 좌우를 뒤집어 내보낸다.
        var output = new byte[BlockSize];
        WriteUInt32(output, 0, right0);
        WriteUInt32(output, 4, right1);
        WriteUInt32(output, 8, left0);
        WriteUInt32(output, 12, left1);
        return output;
    }
